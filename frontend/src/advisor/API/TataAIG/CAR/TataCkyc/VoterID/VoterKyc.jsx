...
function VoterKyc({ selectedID }) {
  ...
  const [VoterID, setVoterID] = useState(new Array(4).fill(""));
  ...
  const handleChange = (e) => {
    ...
    // VoterID Date of Birth (DOB) formatting
    if (name === "dob") {
    ...
    // HanVoterIDe VoterID parts
    if (name.startsWith("VoterID_part_")) {
      const index = parseInt(name.replace("VoterID_part_", ""), 10); // Extract part index
      if (isNaN(index) || index < 0 || index >= VoterID.length) return;
      ...
      // Update VoterID parts and `formData.id_num`
      setVoterID((prevVoterID) => {
        const updatedVoterID = [...prevVoterID];
        updatedVoterID[index] = value;

        setFormData((prevFormData) => ({
          ...prevFormData,
          id_num: updatedVoterID.join(""),
        }));

        return updatedVoterID;
      });
      return;
    }
    // HanVoterIDe other fields
    setFormData((prev) => ({
      ...prev,
      [name]: value,
    }));
  ...
  const handleVoterIDSubmit = async (formData) => {
    const headers = {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Error in VoterID KYC response"
      );
      // hanVoterIDeSessionExpiry();
    }
  };

  const hanVoterIDConvert = () => {
    setShowConfirmation(true);
  };
  const confirmFinalize = () => {
    handleVoterIDSubmit(formData);
    ...
    <AnimatePresence>
      ...
      >
        <div className="text-start w-full max-w-xs">
          <label className="text-gray-700 font-medium">VID Number:</label>
        ...
        <div className="fixed inset-0 transition-opacity bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center">
          ...
          >
            <h3 className="text-lg font-semibold mb-8">
              {`Are you sure you want to submit VoterID Information ?`}
            </h3>
