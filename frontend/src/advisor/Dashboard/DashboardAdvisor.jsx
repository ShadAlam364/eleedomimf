...
function DashboardAdvisor() {
  const dashboardRouted = [
    ...
    },
    {
      ...
      // logo: <RxDashboard size={25} />
      logo: <img src="/account.png" height={10} width={25} alt="policy list" />,
    },
    {
      title: "Insurance",
      path: "#",
  ...
  const names = sessionStorage.getItem("advname");
  return (
    <>
      {/* nav bar */}
          ...
          <div>
            ...
            >
              ...
              <div className="w-12 h-12 rounded-full bg-gray-200 flex items-center justify-center text-blue-800 font-bold text-xl">
                {names ? names.charAt(0) : "A"}
              </div>
            </button>
          </div>
          {/* Dropdown with animation and outside click handling */}
          <div 
            className={`fixed inset-0 z-40 bg-black bg-opacity-0 transition-opacity duration-300 ${isDropdownOpen ? 'opacity-30 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
            onClick={() => setIsDropdownOpen(false)}
            style={{display: isDropdownOpen ? 'block' : 'none'}}
          ></div>
          <div 
            className={`z-50 absolute text-center right-0 top-10 my-5 text-base list-none bg-gradient-to-tl from-blue-800 to-cyan-900 divide-y divide-gray-100 rounded shadow overflow-hidden transition-all duration-300 ${isDropdownOpen ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 -translate-y-2 scale-95'}`}
            style={{display: isDropdownOpen ? 'block' : 'none'}}
          >
            ...
            <ul className="flex justify-center flex-col space-y-2 mx-4 mb-4 text-white" role="none">
              <li>
                <NavLink
                  to="/advisor/home"
