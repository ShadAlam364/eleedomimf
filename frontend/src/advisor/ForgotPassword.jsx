  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${VITE_DATA}/forgot/advisor/password`,
        {
          advisoremail,
        }
      );
      ...
      if (response.status === 200) {
        ...
        toast.error("Advisor Account Not Found!");
      }
    } catch (error) {
      console.log(error.message);
      toast.warn("Advisor Not Registered Yet...! ");
    }
  };
  return (
    <>
      <section className="container-fluid h-screen relative bg-blue-600">
        <div className="container-fluid pt-5 flex flex-col md:flex-row items-center  justify-between bg-blue-600">
          ...
          <div className="flex-shrink-1 px-32  md:h-full h-full w-full xs:w-full -mt-20 sm:w-full md:1/2 mx-auto lg:w-1/2 xl:w-1/2 xl:py-10">
            <div className="w-full max-w-xl p-6  sm:p-10 bg-white rounded-lg shadow ">
              <h2 className="text-2xl font-bold text-gray-900 ">Advisor</h2>
              <h2 className="text-2xl font-bold text-gray-900 ">
                Forgot Password
              </h2>
              <form className="mt-0 space-y-3" method="POST">
                <div>
                  <label
