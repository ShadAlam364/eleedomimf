...
function UpdateDailyVisit({ data, onClosed,fetchData }) {
    ...
    const updateDvr = async () => {
      try {
        ...
        const resp = await axios.put(`${VITE_DATA}/dailyvisit/update/current/${datas._id}`, datas);
          ...
          <div className="relative bg-blue-200 rounded-lg shadow ">
            ...
            <div className="col-span-4 p-2 mt-4 flex justify-center">
                    <button
                      ...
                      onClick={updateDvr} type="button" > {loading ? "Submitting..." : "Submit"} </button>
                  </div>
          </div>
        </div>
      </div>
...
export default UpdateDailyVisit;
