...
function BranchIndorshment() {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          const response = await axios.get(`${VITE_DATA}/indorshment/view/${branch}`, {
            headers: {
              Authorization: `${token}`,
            },
          });
    ...
    const filteredData = indorshData.filter(data => {
      ...
      const subtypess = data.subTypeIndo?.toLowerCase() || "";
      return (
        ...
        (subtypess.includes(subtype.toLowerCase()) || subtype === '')
      );
    });
...
export default BranchIndorshment