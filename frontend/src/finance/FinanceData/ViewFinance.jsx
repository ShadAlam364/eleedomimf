...
function ViewFinance() {
  ...
  const onUpdateInsurance = useCallback(async () => {
    try {
      const token = sessionStorage.getItem("token");
      if (!token) {
  ...
  const handleDateRangeChange = useCallback((event, type) => {
    if (type === "start") {
  ...
  const exportAdvisorWiseReconData = useCallback(() => {
    try {
      const fileType =
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=UTF-8";
      const fileExtension = ".xlsx";
  ...
  const handleAdvisorWiseReconData = useCallback(() => {
    exportAdvisorWiseReconData();
  }, [exportAdvisorWiseReconData]);

  const exportMisToExcel = useCallback(() => {
    try {
      const fileType =
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=UTF-8";
      const fileExtension = ".xlsx";
  ...
  const handleMisExportClick = useCallback(() => {
    exportMisToExcel();
  }, [exportMisToExcel]);
  ...
  return (
    <section className="container relative  p-0 sm:ml-48 bg-slate-200">
      <div className="container-fluid flex justify-center border-gray-200 border-dashed rounded-lg dark:border-gray-700 bg-slate-200">
